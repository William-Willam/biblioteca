...
| email | string | |
| telefone | string | |
| ativo | boolean | Suporta inativação em vez de exclusão (RF02) |
...
| autor | string | |
| isbn | string | UK |
| quantidade_total | int | Não pode ser reduzido abaixo da quantidade já emprestada (RN14) |
| quantidade_disponivel | int | Decrementada no empréstimo, incrementada na devolução |
| ativo | boolean | Suporta inativação em vez de exclusão quando há empréstimo/reserva pendente (RF13) |
...
| data_prevista_devolucao | date | `data_emprestimo + 14 dias` (RN10), ou `+7 dias` em caso de autorreserva (RN13) |
| data_devolucao_real | date | Nulo enquanto o empréstimo está ativo |
| valor_multa | decimal | Nunca `double` — evita erro de arredondamento monetário. Teto de R$20,00 (RN12) |
| multa_paga | boolean | Default `false`. Marcado via endpoint específico (RF12) — só então libera novo empréstimo (RN02) |
...
4. **Sem campo de imagem/capa no Livro** — decisão registrada no levantamento de requisitos (ver RF16 nas melhorias futuras).
5. **`ativo` em Pessoa e Livro, não exclusão física** — ambas as entidades usam soft delete quando há vínculo pendente (empréstimo ativo ou reserva). Exclusão física só é permitida quando não há vínculo algum, evitando registros históricos órfãos por FK quebrada.
6. **`multa_paga` como campo próprio, separado de `valor_multa`** — a multa nasce em aberto na devolução (RN01) e só é quitada por uma ação explícita (RF12), não automaticamente. Isso mantém RN02 funcional: bloquear novo empréstimo enquanto a pendência existir, independente de quando o livro foi devolvido.
